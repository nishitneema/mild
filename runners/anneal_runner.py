...
class AnnealRunner():
    ...
    def anneal_Langevin_dynamics_with_momentum(self, x_mod, scorenet, sigmas, n_steps_each=100, step_lr=0.00002):
        images = []
        ...
        with torch.no_grad():
            for c, sigma in tqdm.tqdm(enumerate(sigmas), total=len(sigmas), desc='annealed Langevin dynamics sampling'):
                ...
                labels = labels.long()
                step_size = step_lr * (sigma / sigmas[-1]) ** 2
                # incrementing gamma according to given step size
                if c>5:
                    gamma+=self.args.step_size_gamma
                for s in range(n_steps_each):
                    images.append(torch.clamp(x_mod, 0.0, 1.0).to('cpu'))
                    noise = torch.randn_like(x_mod) * np.sqrt(step_size * 2)
                    grad = scorenet(x_mod, labels)
    ...
    def anneal_Langevin_dynamics_with_nesterov(self, x_mod, scorenet, sigmas, n_steps_each=100, step_lr=0.00002):
        images = []
        V = torch.zeros_like(x_mod)
        gamma = self.args.gamma
        with torch.no_grad():
            for c, sigma in tqdm.tqdm(enumerate(sigmas), total=len(sigmas), desc='annealed Langevin dynamics sampling'):
                ...
                for s in range(n_steps_each):
                    images.append(torch.clamp(x_mod, 0.0, 1.0).to('cpu'))
                    noise = torch.randn_like(x_mod) * np.sqrt(step_size * 2)
                    ##-------------------------This is where our main contribution comes---------------------------------------
                    # Nesterov update
    ...
    def find_fid(self):
        ...
        output_path = os.path.join(self.args.image_folder, 'T_{}gamma_{}'.format(self.args.T,self.args.gamma))
        ...
        for i in range(num_iters):
            ...
            if self.args.without_mild:
                all_samples = self.anneal_Langevin_dynamics(init_samples, score, sigmas, self.args.T, 0.00002)
                method = 'without_mild'
            elif self.args.mild_momentum:
                all_samples = self.anneal_Langevin_dynamics_with_momentum(init_samples, score, sigmas, self.args.T, 0.00002)
                method = 'mild_momentum'
            else:
                all_samples = self.anneal_Langevin_dynamics_with_nesterov(init_samples, score, sigmas, self.args.T, 0.00002)
                method = 'mild_nesterov'
        ...
        print("Gamma: {}, fid: {}".format(self.args.gamma, fid))
        # Creating a file for appending
        file_path = "fid_results_69.txt"
        with open(file_path, "a") as file:
            file.write("{}, dataset: {}, number of samples: {}, batch_size: {}, T: {}, gamma: {}, step_size: {}, fid: {}\n".format(
                method, self.config.data.dataset, num_samples, batch_size, self.args.T, self.args.gamma, self.args.step_size_gamma, fid))
        # this command will delete the images generated
    ...
    def test(self):
        ...
        if self.config.data.dataset == 'MNIST':
            ...
            if self.args.without_mild:
                all_samples = self.anneal_Langevin_dynamics(samples, score, sigmas, self.args.T, 0.00002)
                method = 'without_mild'
            elif self.args.mild_momentum:
                all_samples = self.anneal_Langevin_dynamics_with_momentum(samples, score, sigmas, self.args.T, 0.00002)
                method = 'mild_momentum'
            else:
                all_samples = self.anneal_Langevin_dynamics_with_nesterov(samples, score, sigmas, self.args.T, 0.00002)
                method = 'mild_nesterov'
        ...
        path = self.args.image_folder+'/'+str(self.args.T)+'_'+str(self.args.gamma)+'_'+str(self.args.step_size_gamma)
